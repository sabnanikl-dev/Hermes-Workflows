...
- `pr-prover/` — a thin, standard-library orchestration tool Hermes uses to prove an existing pull request merge-ready, blocked, or in need of Karan. [`pr-prover/README.md`](pr-prover/README.md) documents how it is operated.
- `Karan-skills/` and `Jake-skills/` — skill snapshots maintained by their own sync workflow. [`Karan-skills/software-development/autonomous-pr-prover/SKILL.md`](Karan-skills/software-development/autonomous-pr-prover/SKILL.md) is the judgment router that sends Hermes into the tool; it holds routing and classification only, never mechanics.
...
- **Integration Auditor** proves the shipped prompt/config/parser/GitHub-readback path works as one contract.
...
Documentation-only and skill-only changes run the same suite, because the router and contract documents are covered by `pr-prover/tests/test_router.py`. Where a change adds or alters a real execution adapter, also run that adapter's repository-owned smoke before the final review sequence. For every push, verify the expected commit appears on the live PR before reporting success. A green suite is necessary but does not override unresolved contract blockers.
