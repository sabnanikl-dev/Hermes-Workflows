...
Reading a bounded revision claim an operator-owned gate wrote down, and reconciling it against the run head, is likewise reporting rather than a deployment integration. `pr-prover` never creates, promotes, or observes an environment; it declines to describe one it cannot bind, and the alternative to that reconciliation is not a smaller claim but an unqualified one.
...
| M13 | Reports are recursively redacted and distinguish transport success, implementation verdict, exact-head readiness, and human merge authority. Every report also states the bounded evidence the run established — when the reported head was observed, what each configured gate's result is evidence about, which adapters and declared runtimes produced the judgments — and names what it did not establish. | shipped | `redaction.py` sanitizes the whole assembled payload; `report.py` renders per-lane transport, `transport_complete`, outcome, head, verdicts, classification, and a constant `merge_authority` line in both renderings; redaction, boundary, and report-separation tests. The disclosure half is bounded the same way: `loop.py` records the UTC observation time in the same statement as the head it dates, so a run that read nothing live reports neither; `config.py` gives a gate an operator-declared `coverage` sentence and an optional `environment` declaration, and every gate reports one of exactly three evidence modes with `local-or-unspecified` as the default every existing configuration keeps; `environment.py` is the only route to `head-bound-environment`, holding one narrow envelope the gate wrote outside its worktree to this run's repo, PR, gate, environment, head, and an externally observed revision equal to that head, validated after the gate's own cleanliness check and stopping the run before any reviewer lane when it cannot be reconciled — so `{head}` or a URL in argv raises nothing by itself, and a failing gate or a later push leaves the endpoint reported unbound; `report.py` carries each validated artifact's `RUNTIME=` claim beside its lane's configured adapter with the shared-adapter and shared-runtime answers, labels coverage and topology as declarations rather than measurements, and states five claims — environment revision binding, heterogeneous reviewer independence, merged-result behavior, deployment/production health, and human final merge review — false unless bounded evidence exists, which for the last four is by construction. Two claims inside that disclosure are held to their own evidence rather than to the mode that carries them. The unbound sentence says a live endpoint *was checked*, which is a statement about execution, so `loop.py` records how far each configured gate's command actually got — `not-run`, `skipped`, `completed`, `failed` — and `report.py` prints `checked` only for a gate that completed. `not checked` is held to its own evidence in turn: a skipped visual gate and a gate a fail-closed stop never reached issued no command, so they say nothing was checked, while a gate that exited non-zero or timed out may have reached the endpoint and failed after it — an HTTP smoke recording `503` and exiting `22` did — so it says whether the endpoint was checked is not established rather than trading the unsupported positive for an unsupported negative, and a later execution state falls to that weaker statement by default. And every operator- and gate-authored string the disclosure prints is rendered single-line and escaped, because the report states its own omissions as `- <claim>: **not established**` bullets a few lines above the evidence it quotes: a `coverage`, `url`, `binding_source`, `invocation`, or `RUNTIME=` value carrying newlines could write bullets of that shape saying `**established**`, directly under the block denying them. `environment.py` refuses such a value at the envelope boundary as well, and holds `observed_at` to a parsed UTC instant in canonical form, so a shape-valid impossible date is not promoted into a head-bound binding's observation time. `tests/test_bounded_evidence.py` drives the positive and negative boundaries through the real config file, envelope validator, loop, and both renderings — including the execution-truth, impossible-timestamp, and forged-bullet regressions — and the integration matrix composes two external gates on one head where only one of them is bound |
