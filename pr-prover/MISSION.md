...
| M4 | A reviewer result counts only when role, verdict, and exact head are valid, and the lane's exit status agrees with its marker. | shipped | `verdicts.py`, `reviewers.py`, `loop.py`, verdict and relay tests |
| M5 | GitHub feedback surfaces are complete or the run fails closed. Missing, null, malformed, truncated, or unknown-completeness thread data is not an empty complete result. | owed by PAPI-97; PAPI-90 ships the conservative stop that stands in for it — any comment or review this run cannot attribute to one of its own lanes stops the fix lane before a builder is launched | `github.py`, `loop.py`, human-feedback stop tests |
...
| M7 | A configured login is not by itself proof that feedback is agent-authored. Exclusion requires positive run-owned evidence; unattributed or colliding-account feedback remains human feedback. | shipped for exclusion by run-owned immutable id — reviewer artifacts and builder fix comments alike are owned only where this run watched the id appear and verified it, and a copy under a publishing login is still feedback; edit-after-publication and the remaining shared-login reconciliation are owed by PAPI-97 | `config.py`, `github.py`, `state.py`, `loop.py`, readback and human-feedback stop tests |
| M8 | Merge-readiness configuration contains exactly Reviewer A, Reviewer B, and Integration Auditor in that order. A missing auditor, duplicate role, or auditor-first order is invalid. | shipped | `config.py`, `loop.py`, configuration tests |
...
| M10 | One configured value is the lane's timeout everywhere it is enforced and reported, and a timed-out lane fails closed rather than having its marker read. Quiet output alone is not failure. | shipped | `commands.py`, `loop.py`, `report.py`, command, trusted-agent, and verdict tests |
...
| M13 | Reports are recursively redacted and distinguish transport success, implementation verdict, exact-head readiness, and human merge authority. | shipped for recursive redaction and for separating lane/artifact transport success from the implementation verdict and from exact-head readiness; explicit human-merge-authority reporting is owed by PAPI-97 | `redaction.py` sanitizes the whole assembled payload; `report.py` renders per-lane transport, `transport_complete`, outcome, head, verdicts, and classification; redaction, boundary, and report-separation tests. `report.py` carries no merge-authority field yet |
