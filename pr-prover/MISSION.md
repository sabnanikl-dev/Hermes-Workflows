...
| M6 | An acknowledgement clears only an existing earlier comment by immutable ID, with chronology evidence; missing, equal, or ambiguous ordering fails closed. | shipped | `feedback.py` holds one finite classifier — six questions, no serial special cases — over candidates ordered globally by UTC-aware timestamp, surface, then id; `loop.py`; the acknowledgement truth-table tests |
