...
Isolated worktrees, process-group cleanup, task-scoped commands, realistic timeouts, redaction, empty optional MCP configuration, and direct GitHub verification are execution hygiene—not a reason to grow the product into a security platform.
...
A merge-readiness run must configure **exactly one** of each required role in that order. Missing, duplicated, or reordered required roles fail closed. Supporting implementation may use generic internal iteration, but configuration cannot redefine the required acceptance lifecycle.
...
| ID | Required behavior | Current informative proof seams |
| --- | --- | --- |
| M1 | Every gate, artifact, classification, and report binds to one full current PR `headRefOid`; live PR number, state, head branch, and base branch remain unchanged at terminal decisions. | `github.py`, `loop.py`, stale-head tests |
| M2 | Any push invalidates all prior evidence and requires gates plus the full ordered review lifecycle on the new head. | `loop.py`, state/verdict tests |
| M3 | A builder push counts only when local worktree HEAD, verified remote branch, PR `headRefOid`, PR commit ancestry/list, and a new signed comment from the configured builder identity agree. | `github.py`, `loop.py`, trusted-agent/readback tests |
| M4 | A reviewer result counts only when role, signature, author, verdict, and exact head are valid; formal reviews use GitHub `commit_id`, while body-bound artifacts contain exactly one standalone canonical `HEAD=<40 lowercase hex>` line. | `reviewers.py`, `verdicts.py`, relay tests |
| M5 | GitHub feedback surfaces are complete or the run fails closed. Missing, null, malformed, truncated, or unknown-completeness top-level thread data is not an empty complete result; nested reply completeness must also be established. | `github.py`, `feedback.py`, pagination/completeness tests |
| M6 | An acknowledgement clears only an existing earlier comment by immutable ID. The target must precede the acknowledgement by GitHub ordering/time evidence; missing, equal, or ambiguous chronology fails closed. | `feedback.py`, human-feedback chronology tests |
| M7 | A configured login is not by itself proof that feedback is agent-authored. Exclusion requires positive run-owned evidence such as expected author, role/signature, new artifact identity, and head binding. Unattributed or colliding-account feedback remains human feedback. | `feedback.py`, `reviewers.py`, identity-collision tests |
| M8 | Merge-readiness configuration contains exactly Reviewer A, Reviewer B, and Integration Auditor in that order. A missing auditor, duplicate role, or auditor-first order is invalid. | `config.py`, `loop.py`, configuration/lifecycle tests |
| M9 | Operational clone state is not mutated. Each lane uses a fresh run-owned worktree at a verified head; unsafe/reused paths and all worktree-root filesystem failures become sanitized fail-closed prover results rather than raw exceptions. | `worktrees.py`, `errors.py`, worktree/integration tests |
| M10 | The enforced lane timeout and the reported timeout are identical. Quiet output alone is not failure, and timeout cleanup terminates the lane process group before returning. | `commands.py`, `loop.py`, process tests |
| M11 | State and lock creation/read/write/replace/cleanup failures are deterministic, sanitized, and fail closed while preserving the original stop reason. | `state.py`, `errors.py`, state tests |
| M12 | The normal fix budget is two cycles; attempt three is unreachable without an explicit Karan exception recorded outside the automatic path. | `state.py`, `loop.py`, cycle-cap tests |
| M13 | Reports are recursively redacted and distinguish transport success, implementation verdict, exact-head readiness, and human merge authority. | `redaction.py`, `report.py`, boundary tests |
| M14 | The tool never merges, force-pushes, deploys, installs, releases, or mutates client/live/account systems. | reachable command surfaces, boundary scans/tests |
...
Reviewer A focuses on correctness, safety, failures, tests, and regressions. Reviewer B focuses on architecture, maintainability, mission drift, and proportionality. Integration Auditor checks the shipped configuration, prompts, adapters, parsers, relay/readback, GitHub surfaces, and exact-head outcome as one end-to-end contract.
...
- missing deterministic proof for a load-bearing invariant;
- scope contamination or prohibited authority.

Alternative designs, generalized hardening, style preferences, and hypothetical future-agent requirements are non-blocking. Reviewers must not revive the superseded zero-trust platform or demand implementation-total formality beyond this behavioral contract.
...
- M1–M14 have executable coverage appropriate to the changed surfaces;
