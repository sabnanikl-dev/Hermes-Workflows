...
- **Toolset present but not exposed in the current session** — Do not infer
  active computer-use capability from the skill library alone. Verify with
  `hermes tools list | grep -i computer`. If `computer_use` is disabled,
  run `hermes tools enable computer_use`, verify it is enabled, and tell the
  user a fresh session or `/reset` is required before the actual
  `computer_use(...)` tool appears in the conversation tool list.
