...
## Umbrella Scope: MCP Server Operations

This is the class-level skill for ad-hoc MCP server listing, configuration, auth, and calls. The absorbed `antigravity-mcp-config` skill is preserved in `references/antigravity-mcp-config.md` as an IDE-specific MCP configuration subsection: avoid Docker when unavailable, account for macOS GUI PATH inheritance, use absolute command paths when needed, and test server startup outside the IDE before blaming MCP itself.
