...
```bash
pr-prover check-config --config <run.json>   # identities, roles, budgets
pr-prover run --config <run.json> --json     # 0 merge-ready | 1 blocked | 2 needs-Karan
```

Write `run.json` from `pr-prover/examples/run.example.json`: the PR, the clone to borrow objects from, gates, one argv array per trusted agent, and the login each must publish under; `pr-prover/README.md` documents the output and artifact contract each lane must satisfy. The tool binds the run to the exact `headRefOid`, verifies every builder and reviewer claim through GitHub readback, and stops and asks rather than guessing. Hermes' judgment sits either side of it: write the config, read the report, advise Karan.
...
- **Codex Reviewer A/B and Integration Auditor** — independent review lanes. Inspect one exact head and publish their own role-signed artifacts under the configured reviewer login.
...
Quiet is not stuck: a trusted lane may print nothing for twenty minutes. Read the tool's lane observations before believing a lane is hung.
