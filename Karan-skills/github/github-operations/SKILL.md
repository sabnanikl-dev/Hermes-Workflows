...
8. If the repository should retain only **recently used** local skills, define usage from durable execution evidence rather than file modification time: successful `skill_view` results in the default and all specialist-profile `state.db` session stores, plus skills explicitly attached to cron jobs whose `last_run_at` falls inside the rolling window. Intersect those names with Hermes-local provenance before copying. Fail closed if session stores are unavailable or the intersection is unexpectedly empty; handle nested umbrella/child skill directories so an unused child is not copied merely because its used parent was copied. Keep the manifest deterministic by recording the window policy/evidence types but not generation time, moving cutoff, usage counts, or last-used timestamps. Test recent/stale/failed/never-run cases with fixed-time fixtures, run the real sync twice, and verify the second run produces an identical diff hash.
