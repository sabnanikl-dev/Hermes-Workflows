...
The community corpus (see `references/absorbed/pretext/references/patterns.md`) clusters into a handful of strong patterns. Pick one and riff — don't invent a new category unless asked.
...
See `references/absorbed/pretext/templates/donut-orbit.html` and `references/absorbed/pretext/templates/hello-orb-flow.html` for working single-file starters.

## Workflow

1. **Pick a pattern** from the table above based on the user's brief.
2. **Start from a template**:
   - `references/absorbed/pretext/templates/hello-orb-flow.html` — text reflowing around a moving orb (reflow-around-obstacle pattern)
   - `references/absorbed/pretext/templates/donut-orbit.html` — advanced example: measured ASCII logo obstacles, draggable wire sphere/cube, morphing shape fields, selectable DOM text, and dev-only controls
