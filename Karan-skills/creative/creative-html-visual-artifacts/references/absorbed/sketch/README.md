...
## Scoped component sketches

When the user asks for options for an existing section/card/component, preserve the current component boundary by default. Do **not** redesign the surrounding page or invent a larger interaction model unless the user asks for a broader concept pass.

Examples:
- “I don’t like the hours section/card” in an existing site usually means: vary the hours display inside the current card footprint, keep the address/services cards and section layout intact.
- “More options with how X is displayed” means: compare display treatments for X, not new flows, dashboards, planners, or full-page concepts.

For bounded component work:
1. Inspect the existing implementation and identify the smallest relevant container.
2. Show the current context briefly, then place variants inside the same footprint/visual language.
3. Keep exports focused on the chosen element: HTML snippet, CSS snippet, Markdown instructions, and JSON summary if useful.
4. If the user selects a winner and asks for a PR/design-doc artifact, commit only the selected option’s relevant files unless they explicitly ask to preserve all explored options.

## Output

- Default lightweight location: create `sketches/` (or `.planning/sketches/` if the user is using GSD conventions) in the repo root.
- If the repo already has a design-doc convention such as `docs/design/`, prefer putting reviewable sketches there instead, especially when the sketch will be committed or referenced by GitHub issues/PRs. For JMD website work, use `docs/design/<sketch-name>/`, not repo-root `sketches/`.
- One subdir per variant or comparison package: `NNN-stance-name/index.html` + `README.md`, or a named comparison folder with `exports/` when the user wants implementation-ready issue guidance.
- When exporting implementation guidance, include only files relevant to the current ask/selected option unless the user asked to archive all options:
  - `option-name.html` — focused markup/snippet or minimal preview
  - `option-name.css` — isolated CSS using project tokens where possible
  - `option-name.md` — concise implementation notes and guardrails
  - `option-name.json` — structured summary with id/name/scope/components/constraints/trade-offs
- Tell the user how to open them: `open docs/design/<sketch-name>/index.html` or `open sketches/001-calm-editorial/index.html` on macOS, `xdg-open` on Linux, `start` on Windows.
