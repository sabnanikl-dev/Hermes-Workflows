...
## Client-Facing Audit / Report Presentations

When converting an internal audit, ops harness output, Linear issue, or evidence folder into a client-facing HTML presentation, use `references/absorbed/claude-design/references/client-facing-audit-presentations.md`.

For internal operational dry runs, collector-script outputs, memory closeouts, or source-health probes where the deliverable is a reviewable HTML report plus raw evidence, use `references/absorbed/claude-design/references/operational-dry-run-html-reports.md`. That pattern emphasizes deterministic read-only collection, inline SVG diagrams for routing/decision flow, source-health disclosure, and Telegram-safe packaging.

Key rules:

- Extract the business decision and next actions, not the internal workflow chronology.
- Remove internal/tooling details such as agent names, auth-token failures, raw repo chatter, and evidence filenames unless the user explicitly wants audit traceability shown.
- Convert long audit matrices into concise executive sections: verdict, key findings, priority fixes, baseline, approval checklist, rollout.
- Keep approval gates visible when live public changes are involved.
- Verify with a browser when possible; if `file://` is blocked, serve the deliverable from a temporary local HTTP server and check console errors.
- Add a data-URI favicon or favicon file for standalone reports to avoid easy 404 console noise.
...
For HTML slide decks with fixed or sticky navigation, screenshot/visual-QA the opening viewport specifically for nav overlap. If controls cover hero copy, move them to a corner or reduce their footprint before delivery.
...
### Delivering artifacts over messaging platforms

When the user asks to review a generated artifact on their phone or says to “send” the file in chat, do not merely print an on-disk path. In Telegram or other gateway chats, include one `MEDIA:/absolute/path` line per file so the gateway can upload it as an attachment.

For standalone HTML/report artifacts, **always send a `.zip` fallback in the same first delivery**, not only after the user complains. Some Telegram/mobile clients do not reliably surface loose `.html` attachments even when a `MEDIA:` line is present. Copy the HTML into a simple review folder with a human-readable filename, zip that folder/file, verify both files are nonzero, then attach both.

Recommended pattern:

```text
MEDIA:/absolute/path/<project>-review/Report.html
MEDIA:/absolute/path/<project>-review.zip
```

If the user says the file did not arrive, do not repeat the same lone attachment. Repackage immediately: create a fresh review folder, copy the file with a clean filename, create a zip fallback, verify sizes, and send both `MEDIA:` lines.

For Telegram/gateway delivery, stage attachments in a gateway-approved Hermes cache path such as `~/.hermes/cache/documents/<project>-review/` or `~/.hermes/cache/documents/<project>-review.zip` before emitting `MEDIA:` lines. `~/Downloads` is fine for local review copies, but may not attach correctly through the gateway. Verify each staged file exists and has nonzero size before final response.
