...
prerequisites:
  commands: ["python3"]
setup:
  help: "Run references/absorbed/comfyui/scripts/hardware_check.py FIRST to decide local vs Comfy Cloud; then references/absorbed/comfyui/scripts/comfyui_setup.sh auto-installs locally (or use Cloud API key for platform.comfy.org)."
metadata:
  hermes:
    tags:
      - comfyui
...
**Reference docs (`references/absorbed/comfyui/references/`):**
...
**Scripts (`references/absorbed/comfyui/scripts/`):**
...
python3 references/absorbed/comfyui/scripts/hardware_check.py
```
...
```bash
python3 references/absorbed/comfyui/scripts/health_check.py
...
```bash
python3 references/absorbed/comfyui/scripts/extract_schema.py workflow_api.json --summary-only
# → {"parameter_count": 12, "has_negative_prompt": true, "has_seed": true, ...}

python3 references/absorbed/comfyui/scripts/extract_schema.py workflow_api.json
...
python3 references/absorbed/comfyui/scripts/run_workflow.py \
...
python3 references/absorbed/comfyui/scripts/run_batch.py \
  --workflow sdxl.json \
...
| "install ComfyUI" | comfy-cli | `bash references/absorbed/comfyui/scripts/comfyui_setup.sh` |
...
```bash
python3 references/absorbed/comfyui/scripts/hardware_check.py --json
# Optional: also probe `torch` for actual CUDA/MPS:
python3 references/absorbed/comfyui/scripts/hardware_check.py --json --check-pytorch
```
...
```bash
bash references/absorbed/comfyui/scripts/comfyui_setup.sh
# Or with overrides:
bash references/absorbed/comfyui/scripts/comfyui_setup.sh --m-series --port=8190 --workspace=/data/comfy
```
...
python3 references/absorbed/comfyui/scripts/check_deps.py my_workflow.json
...
python3 references/absorbed/comfyui/scripts/fetch_logs.py --tail-queue --host https://cloud.comfy.org
```
...
Use `python3 references/absorbed/comfyui/scripts/health_check.py` to run the whole list at once. Manual:
