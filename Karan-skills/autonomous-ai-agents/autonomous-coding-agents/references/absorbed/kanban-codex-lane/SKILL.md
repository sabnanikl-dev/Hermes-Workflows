...
Use the linked template at `references/absorbed/kanban-codex-lane/templates/pmb-codex-lane-prompt.md` for prediction-market-bot work. For other repos, keep the same structure and replace the PMB-specific safety block with repo-specific invariants.
...
- [ ] Hermes ran the canonical tests itself, using `references/absorbed/kanban-codex-lane/scripts/run_tests.sh` for Hermes Agent or the repo's documented wrapper for other repos.
...
```json
{
  "codex_lane": {
    ...
    "tests_run": [
      {"command": "references/absorbed/kanban-codex-lane/scripts/run_tests.sh tests/tools/test_x.py", "exit_code": 0, "owner": "hermes"},
