...
1. **Interactive mode usually uses tmux, but tmux may be absent** — Claude Code is a full TUI app. Prefer tmux when installed because `capture-pane`/`send-keys` make orchestration reliable. If `tmux: command not found`, fall back to Hermes background PTY (`terminal(..., pty=true, background=true)`) plus `process.poll`/`process.write`/`process.kill`. This is workable for simple prompts, trust dialogs, and auth diagnosis, but less reliable for long reviews.
2. **`--dangerously-skip-permissions` dialog defaults to "No, exit"** — you must send Down then Enter to accept. Print mode (`-p`) skips this entirely.
3. **`claude auth status` can be a false positive** — it may show a valid Claude Pro login while actual `claude -p` or interactive requests fail with `API Error: 401 Invalid authentication credentials` / `Please run /login`. Before claiming Claude reviewed anything, run a tiny live probe such as `claude -p 'Say hi' --max-turns 1`. If the probe fails, do not post a fake Claude review; tell the user re-auth is required and offer an alternate reviewer clearly labeled as non-Claude.
...
10. **Background tmux/PTTY sessions persist** — always clean them up (`tmux kill-session -t <name>` or `process.kill`) when done to avoid resource leaks.
