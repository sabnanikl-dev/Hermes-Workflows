...
When adding a Linear comment as evidence for a verification gate, do not rely only on `comments(last:1)` to prove your comment is visible. Linear comment ordering can make the newest-created comment not appear as `last:1` in the shape you expect. Capture the `commentCreate.comment.id`, then verify it directly with `comment(id: ...)` and confirm the body contains the key evidence string (for example a SHA-256 or command name) before reporting success.

When a review finds that an issue's boxes are ahead of real operational evidence (for example a scheduled run has only been proven by a controlled/manual execution), do not paper over the gap with wording alone. Preserve the caveat in Linear, then create a narrow read-only follow-up verifier: a script-only/no-agent cron watchdog that stays silent while waiting, checks the exact missing evidence class (execution mode/status, post-run counts, duplicate checks), and emits a single final success/failure/deadline report. Keep it non-mutating and redacted, then use the watchdog output as the final Linear follow-up note.

For production-readiness split-outs or follow-up issues created from an active parent issue, preserve the parent context explicitly: create the child with `--parent`, then copy the parent project, assignee, and relevant label IDs with a raw `issueUpdate` mutation if the helper's `create-issue` command cannot set them. Add a parent comment that links the new child and names the operational caveat, capture the returned `comment.id`, then verify both the child issue shape and the parent comment via a direct `comment(id: ...)` query before reporting success. This prevents a critical caveat issue from landing in an unassigned/no-project backlog and makes the activation/readiness boundary visible from the parent.
