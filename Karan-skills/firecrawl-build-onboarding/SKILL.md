...
## Agent/Headless CLI Init Notes

In agent/headless environments, `npx -y firecrawl-cli@latest init --all --browser` may hang while waiting for browser authentication, even if it prints a login URL. If an API key already exists, skip browser auth by passing it explicitly:
...
After install, verify the CLI and auth before reporting success:

```bash
command -v firecrawl
firecrawl --version
set -a; source ~/.hermes/.env; set +a
firecrawl -k "$FIRECRAWL_API_KEY" --status
```

Check the status output for both authentication and available credits. A successful CLI install can still be unusable for scraping/searching if Firecrawl reports 0 credits left.
