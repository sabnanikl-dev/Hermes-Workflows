...
- `references/exact-head-responsive-lazy-image-proof.md` — exact-PR-head screenshot pattern for responsive pages with lazy remote images: avoid raw Chrome minimum-width cropping and one-shot full-page fallback captures; use scripted Playwright scrolling, image-dimension/overflow assertions, fresh head-labeled artifacts, and a broader source-packet/spec drift scan before current-head A/B re-review.
- `references/browser-qa-evidence-producer-validator-parity.md` — use when a PR adds a real-browser evidence producer plus offline validator: execute the committed producer from a clean checkout, complete every scoped matrix slot, pin classifier identity fields to the authoritative candidate contract, require browser methods for failures, keep the full candidate universe distinct from a `--models` work subset, and prove producer/validator parity with both a two-batch resume regression and a full-scale offline fixture.
...
- No hardcoded hex colors when the target repo defines design tokens — use that repo's documented token system. `femme-*` tokens are Femme Events-specific and must never be imposed on JMD or another project unless its repo guidance explicitly adopts them.
